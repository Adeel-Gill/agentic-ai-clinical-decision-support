...
| P017 | 2023 | Towards Generalist Biomedical AI | Healthcare | Most medical AI models are unimodal, single-task specialist systems that cannot flexibly integrate diverse multimodal patient data or engage in collaborative clinical dialogue. | Med-PaLM M, a large multimodal generative model that acts as a generalist biomedical AI system to encode and interpret clinical language, imaging, and genomics using a single set of model weights. | No | No | Yes – Exhibits zero-shot multimodal chain-of-thought (CoT) reasoning to identify and describe medical findings step-by-step. | No | No | No | Yes | Evaluated quantitatively on the 14-task MultiMedBench against specialist state-of-the-art models and qualitatively through blinded radiologist assessments of generated radiology reports. | The need for large-scale multimodal datasets to jointly scale modality-specific encoders with language models, and the necessity of validating safety and equity in real-world clinical applications. | 10 | Provides a foundational multimodal architecture capable of seamlessly integrating diverse patient data streams (images, clinical notes, genomics) into a unified reasoning engine for comprehensive clinical decision support. |