...
| P020 | 2026 | AI Agents vs. Agentic AI: A Conceptual taxonomy, applications and challenges | Survey | Existing literature conflates basic AI Agents with advanced Agentic AI, causing conceptual ambiguity and misaligned system design for complex, dynamic scenarios. | A structured conceptual taxonomy and architectural analysis that clearly distinguishes single-entity AI Agents from orchestrated, collaborative multi-agent Agentic AI ecosystems. | Yes – Analyzes episodic, semantic, and vector-based persistent memory architectures essential for distributed state management and long-horizon tasks. | Yes – Explores goal decomposition, multi-step execution, and causal simulation-based planning governed by orchestrator meta-agents. | Yes – Discusses the progression from heuristic reasoning to causal modeling, tool-augmented reasoning, and reflexive self-evaluation. | External APIs, web search, function calling, vector databases (e.g., Pinecone), and code execution. | Yes | Yes | Yes | Conducts a comprehensive literature-based evaluation to construct a comparative conceptual taxonomy rather than empirical benchmarking. | The critical need for integrating causal inference, standardized multi-agent coordination protocols, and governance-aware architectures for deployment in high-stakes domains. | 9 | Provides the foundational architectural taxonomy and multi-agent coordination frameworks (orchestration, role specialization, and shared memory) essential for structuring a robust intelligent patient monitoring and clinical decision support system. |