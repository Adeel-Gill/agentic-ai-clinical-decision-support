...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 002 | 2023 | Generative Agents: Interactive Simulacra of Human Behavior | Agent Architecture | Simulating believable human behavior over long-term horizons is hindered by the limited context windows of LLMs and the need for persistent coherence across thousands of interactions. | A novel architecture that combines an LLM with a long-term natural language "memory stream," a "reflection" module for higher-level inferences, and a recursive "planning" module. | Yes – A comprehensive natural language "memory stream" utilizing a retrieval function based on recency, importance, and relevance to surface relevant experiences. | Yes – A recursive top-down mechanism that generates high-level daily agendas and decomposes them into granular, moment-to-moment action plans. | Yes – A periodic reflection process that synthesizes raw observational memories into higher-level, abstract insights to guide long-term behavior and inferences. | No | Yes | No | No | Human evaluators ranked the believability of agent behavior across ablated architectures and human baselines using natural language "interviews" covering self-knowledge, memory, and planning. | Reducing high computational costs and latency for large-scale simulations while ensuring robustness against prompt hacking and long-term hallucinations. | 9 | Offers a vital architectural template for persistent memory and reflection modules needed to synthesize longitudinal patient history into high-level clinical insights for decision support. |