...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 001 | 2022 | ReAct: Synergizing Reasoning and Acting in Language Models | Framework | Reasoning and acting are typically studied in isolation, leading to hallucinations in reasoning-only models and a lack of high-level goal planning in acting-only models. | A general paradigm that prompts LLMs to generate interleaved verbal reasoning traces and task-specific actions to synergize internal knowledge with external information. | Yes – Maintains working memory through reasoning traces that track progress, update context, and handle exceptions throughout a task trajectory. | Yes – Decomposes high-level goals into subgoals, creates actionable plans, and dynamically adjusts strategies based on environmental observations. | Yes – Generates human-like verbal thoughts to induce plans, extract information from observations, and synthesize final decisions using internal and external knowledge. | Wikipedia API (search, lookup), web navigation tools, and interactive game environments. | No | Yes | No | Performance was measured using success rates and accuracy across four diverse benchmarks (HotpotQA, Fever, ALFWorld, WebShop) compared to reasoning-only and acting-only baselines. | Limited in-context support for complex reasoning and acting behaviors, requiring scaling with multi-task training and reinforcement learning. | 10 | Establishes the core Reason-Act loop required for the agentic framework to interact with patient data while grounding clinical decisions in reasoning and external medical knowledge. |