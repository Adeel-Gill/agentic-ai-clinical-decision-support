...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P004 | 2023 | Toolformer: Language Models Can Teach Themselves to Use Tools | Agentic AI | Language models struggle with precise arithmetic and up-to-date factual lookups, and existing tool-use methods are either human-annotation intensive or task-specific. | A self-supervised model that learns to autonomously call external APIs (search, calculator, etc.) and integrate results into its reasoning process. | No | No | Yes – Employs a self-supervised loss mechanism to autonomously determine when an API call is helpful for accurate token prediction. | Calculator, Q&A system, Wikipedia search engine, machine translation system, and a calendar. | No | Yes | Yes | Demonstrated substantially improved zero-shot performance across math, question-answering, and temporal benchmarks, outperforming much larger models like GPT-3. | Inability to engage in interactive tool use, use tools in a chain (output-to-input), or account for tool-dependent computational costs. | 8 | Provides a self-supervised framework for agents to autonomously leverage external clinical tools and medical databases to ground decision support in verified facts. |