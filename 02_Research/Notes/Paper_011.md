...
| P011 | 2024 | Agent Hospital: A Simulacrum of Hospital with Evolvable Medical Agents | Agent Architecture | Modeling the acquisition of medical expertise through practice rather than just static textbook knowledge. | Agent Hospital, a simulacrum using the Simulacrum-based Evolutionary Agent Learning (SEAL) paradigm to enable autonomous agent evolution. | Yes – Personalized medical case base for successes and an experience base for rules derived from reflective error analysis. | Yes – Simulates full clinical workflows including triage, consultation, medical examinations, and post-hospital follow-ups. | Yes – Clinical reasoning for diagnosis and examination selection using Chain-of-Thought and RAG. | Medical knowledge bases (Baidu Health Encyclopedia), map editor (Tiled), and game development framework (Phaser). | Yes | Yes | Yes | Evolved agents were tested on MedQA (USMLE) and virtual tasks, outperforming state-of-the-art baselines like Medprompt and MedAgents. | Non-evolvable base models, limited to high-level treatment recommendations, and lack of inter-departmental collaboration. | 10 | Provides a complete framework for simulating hospital workflows and an autonomous mechanism for evolving medical agents to reach expert-level proficiency. |